...
// Constants
const DEFAULT_TIMEOUT_MS = 30000; // 30 second timeout per stock
const BATCH_FETCH_LIMIT = 5; // Process 5 stocks in parallel to avoid overwhelming API
 ...
 * @private
 */
async function processStockWithScore(symbol, days, interval, universeMap, includeIndicators) {
  ...
  try {
    ...
    const data = await withTimeout(fetchStockData(symbol, days, true, interval));
    ...
    logger.debug(`Data fetched for ${symbol}`, { symbol, dataPoints: data.length, interval });
    ...
    const name = stockInfo?.name || symbol.replace('.NS', '').replace('.BO', '');
 ...
 * @private
 */
async function processStocksInBatches(symbols, days, interval, universeMap, includeIndicators, progressCallback) {
  const results = [];
  ...
  for (let i = 0; i < symbols.length; i += BATCH_FETCH_LIMIT) {
    ...
    const batchResults = await Promise.all(
      batch.map(symbol => processStockWithScore(symbol, days, interval, universeMap, includeIndicators))
    );
    ...
    if (progressCallback) {
      progressCallback(Math.min(i + BATCH_FETCH_LIMIT, symbols.length), symbols.length);
    }
  }
...
async function scanStocks(stocks, options = {}) {
  ...
  if (!stocks || (Array.isArray(stocks) && stocks.length === 0)) {
    logger.error('Validation failed: Stock list cannot be empty', new Error('Empty stock list'));
    throw new Error('Stock list cannot be empty');
  }

  if (typeof days !== 'number' || days <= 0) {
    logger.error('Validation failed: Days must be positive', new Error('Invalid days parameter'));
    throw new Error('Days must be a positive number');
  }

  const validIntervals = ["1d", "1wk", "1mo"];
  if (!validIntervals.includes(interval)) {
    logger.error('Validation failed: Invalid interval', new Error(`Invalid interval: ${interval}`));
    throw new Error(`Invalid interval. Yahoo Finance only supports: ${validIntervals.join(", ")}`);
  }

  if (typeof minScore !== 'number' || minScore < 0 || minScore > 100) {
    logger.error('Validation failed: minScore out of range', new Error('Invalid minScore parameter'));
    throw new Error('minScore must be a number between 0 and 100');
  }

  if (typeof maxResults !== 'number' || maxResults <= 0) {
    logger.error('Validation failed: maxResults must be positive', new Error('Invalid maxResults parameter'));
  ...
  const progressCallback = (processed, total) => {
    process.stdout.write(`\r  Progress: ${processed}/${total} (${((processed/total)*100).toFixed(0)}%)`);
  };
  ...
  logger.info('Scan completed successfully', {
    ...
    topStocks: topStocks.slice(0, 5).map(s => ({ symbol: s.symbol, score: s.score }))
  });
